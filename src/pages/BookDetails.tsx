...
// const BookDetails: React.FC = () => {
//   const { id } = useParams(); // Specify that id may be undefined
//   const dispatch = useDispatch();
//   const bookDetailsState = useSelector((state: RootState) => state.bookDetails);

//   useEffect(() => {
//     if (id) {
//       // Fetch book details when the component mounts, only if id is not undefined
...
const BookDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      // Fetch book details only if id is not undefined
      dispatch(fetchBookDetails({ id } as any)); // Use non-null assertion here
    }
  }, [dispatch, id]);

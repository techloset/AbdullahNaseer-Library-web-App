    ...
    <div className='parent bg-[#183B56] h-full mt-7 md:mt-24 md:px-36'>
      <div className='flex justify-center md:justify-between pt-12 md:pt-14'>
        ...
        <div className='buttons hidden md:flex '>
          <button className='h-14 w-14 border-2 mx-3 p-2 rounded-full' onClick={scrollLeft}>
            <img className='mx-auto' src={arrowLeft} alt='Left Arrow' />
          </button>
          <button className='h-14 w-14 border-2 mx-3 p-2 rounded-full' onClick={scrollRight}>
      ...
      {!isLoading && !isError && (
        <div className='slider mt-10 md:mt-14 overflow-x-auto whitespace-nowrap mx-auto no-scrollbar' ref={sliderRef}>
          {books.map((book: any) => (
